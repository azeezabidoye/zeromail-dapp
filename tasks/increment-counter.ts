...
task(
  "increment-counter",
  "Increment the counter on the deployed contract",
  ...
  if (!counterAddress) {
    ...
    console.error(
      `Please deploy first using: npx hardhat deploy-counter --network ${network.name}`,
    );
    return;
  }
  ...
  await cofhejs_initializeWithHardhatSigner(_, signer);
